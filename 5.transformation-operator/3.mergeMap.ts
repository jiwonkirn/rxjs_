 ...
 * mergeMap을 사용하면 project 함수에서 리턴하는 Observable 객체를 구독해 값을 각각 발행할 수 있다.
 */
 ...
 * complete 함수를 호출한다.
 * project에서 리턴하는 객체를 구독할 때는 subscribeToResult 라는 함수를 사용하는데,
 ...
 */
range(0, 3)
  .pipe(mergeMap(v => new Promise(res => setTimeout(() => res(`req${v + 1}`), Math.floor(Math.random() * 2000)))))
  .subscribe(v => console.log(`response from ${v}`))
 ...
 * 옵저버블중 하나라도 구독을 해제한다면 버퍼에 저장한 순서대로 값을 하나씩 꺼내서 project 함수에서 옵저버블을 만들어 구독한다.
