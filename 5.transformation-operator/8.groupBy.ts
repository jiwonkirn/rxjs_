...
/**
 * groupBy 연산잔ㄴ 고그 옵저버블에서 발행하는 값을 특정 기준을 정해 같은 그룹에 속해 있는 값들을 각각의 옵저버블로 묶어서 발행한다.
 * 그룹을 묶는 기준: 키
...
/**
  result:
  subjectSelector 함수는 rxjs에서 제공하는 서브젝트 타입의 인스턴스를 리턴할 수 잇는 함수다.
...
/**
 * keySelector 함수로 계산한 키에 해당하는 값을 발행할 소스옵저버블이 존재하지 않을 때 옵저버블의 값릏 전달한다.
